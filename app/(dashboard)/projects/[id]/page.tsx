...
export default async function ProjectPage({
  ...
  const project = projects.find((t) => t.name.toLowerCase().replace(/\s+/g, '-') === id || t.name.toLowerCase() === id);
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="flex gap-2">
            <div className="flex items-center justify-center w-full text-2xl py-4 font-mono rounded-full cursor-pointer bg-black text-white hover:bg-black/80 transition-colors">
              <a href={project.repo} className="flex items-center gap-2" target="_blank" rel="noreferrer">
                Repo <GithubIcon size={32} />
              </a>
            </div>
            {project.website && (
